...
let jsonEmoji = JSON.stringify(EmojiListJson)
const emoji = JSON.parse(jsonEmoji)
var emojiLenght = Object.keys(emoji).length
...
let jsonCountries = JSON.stringify(countryListJson)
const countries = JSON.parse(jsonCountries)
var countriesLenght = Object.keys(countries).length

// const cities = ['Blackpool', 'London', 'Manchester', 'Liverpool', 'Leeds', 'Bristol']
const nicknames = ['lad', 'bloke', 'man', 'guy', 'boy', 'hung', 'slut', 'bitch', 'whore', 'angel', 'brit', 'slave', 'daddy', 'asian', 'american', 'bubble', 'Honey', 'otter', 'dad', 'twink', 'bear', 'hun', 'chub', 'jock', 'chaser']
const adj = ['fuzzy', 'hairy', 'discret', 'dirty', 'clean', 'adventurous', 'caring', 'shaven', 'smooth', 'thicc', 'thick', 'slim', 'sugar', 'filthy', 'bored', 'horny', 'flexible']
const time = ['TODAY', 'TONIGHT', 'THIS EVENING', 'Meet later', 'Right now']
const topBottom = ['top', 'tp', '4top', 'bottom', 'bttm', '4bttm']
const hey = ['hey', 'heya', 'hello', 'hi', 'Yas']
const initials = ['G', 'F', 'T', 'M', 'H']
const locations = ['local', 'New to the area']
const accom = ["can accom", "can accomodate", "can't accom", "can't accomodate", "ACCOM", "NO ACCOM"]
const preMade = ['mouth 4 use', 'HnH', 'Head', 'BDSM', 'Maso', 'Use me', 'Masc', 'masc4masc', 'Femme', 'Femm', 'Pop me', 'Huge', 'Anyone', 'Hi hun', 'Honey', 'Guy next door', 'Boy next door']
const signs = ['!', '?']
...
function checkUndefined(a) {
   function randomValue(b) {
      ...
      if (b !== undefined && b < 9007199254740992) {
         return random.integer(1, b);
      }
   }
   function makeRandom(i) {
      if (i != undefined) {
         return i[randomValue(i)]
      } 
   }
   if (a == emoji && a !== undefined) {
      result = a[randomValue(a)].char
      if (result == undefined) {
         a[randomValue(a)].char
      } else {
         return result
      }
   } else if (a == countries && a !== undefined) {
      result = a[randomValue(a)].name
      if (result == undefined) {
         a[randomValue(a)].name
      } else {
         return result
      }
   } else if (a !== undefined) {
...
let randoEmoji = checkUndefined(emoji)
let randoCountry = checkUndefined(countries)
let randoNickname = checkUndefined(nicknames)
let randoAdj = checkUndefined(adj)
let randoTime = checkUndefined(time)
let randoTopBottom = checkUndefined(topBottom)
let randoHey = checkUndefined(hey)
let randoInitials = checkUndefined(initials)
let randoLocations = checkUndefined(locations)
let randoAccom = checkUndefined(accom)
...
// myArray[3] = nicknames
// myArray[4] = randoTime
// myArray[5] = randoTopBottom
// myArray[6] = randoHey
// myArray[7] = randoInitials
// myArray[8] = randoLocations
// myArray[9] = randoAccom

console.log(randoEmoji)
console.log(randoCountry)
console.log(randoNickname)
